    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow-sm sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">BangStock <span className="text-orange-500">Deals</span></h1>
            <p className="text-sm text-gray-500">Clearance Sale — Limited Stock</p>
          </div>
          <a href="/shop" className="text-sm text-indigo-600 font-medium">View Regular →</a>
        </div>
        {categories.length > 1 && (
          <div className="container mx-auto px-4 pb-3 flex gap-2 overflow-x-auto">
            {categories.map(c => (
              <button key={c} onClick={() => setCategory(c)}
                className={`px-4 py-1.5 rounded-full text-sm font-medium whitespace-nowrap ${category === c ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-600'}`}>
                {c}
              </button>
            ))}
          </div>
      ...
      <main className="container mx-auto px-4 py-6">
        {filtered.length === 0 ? (
          <div className="text-center py-16 text-gray-500">No clearance items right now</div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {filtered.map((product) => (
              <div key={product.id} onClick={() => { setSelectedProduct(product); setSelectedImage(product.image_url) }}
                ...
                <div className="absolute top-2 left-2 bg-orange-500 text-white text-xs font-bold px-2 py-0.5 rounded-full z-[1]">DEAL</div>
                ...
                <div className="p-3">
                  ...
                  <p className="text-base font-bold text-orange-600 mt-1">₹{product.selling_price}</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-white w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex gap-2 px-4 py-2 overflow-x-auto">
                {selectedProduct.product_images.map((img, i) => (
                  <img key={i} src={img.url} onClick={() => setSelectedImage(img.url)}
                    className={`h-14 w-14 object-cover rounded-lg cursor-pointer flex-shrink-0 border-2 ${selectedImage === img.url ? 'border-orange-500' : 'border-transparent'}`} />
                ))}
              </div>
            )}
            ...
            <div className="p-4">
              ...
              <p className="text-2xl font-bold text-orange-600 mt-2">₹{selectedProduct.selling_price}</p>
              ...
              <a href={`https://wa.me/918971170118?text=Hi! I'm interested in ${encodeURIComponent(selectedProduct.name)} (Clearance)`}
